...
bool G(mode) = false;
static bool G(press)[G(_ARRAY_SIZE)];
...
static void romaji_clear(void) {
    G(consonants) = 0;
    //memset( G(press)     ,0,sizeof(bool)*G(_Y) );
    memset( G(press)     ,0,sizeof(bool)*G(_ARRAY_SIZE) );
}
...
static const char* G(shifted_chrs) = "     p b  g  N b  zd      ";
...
static void romaji_flush(void) {
    ...
    if( G(consonants) != 0 ) {
        if( fshift  ) {
            ch = G(shifted_chrs)[ G(consonants) - KC_A ];
        }
        else {
